    ...
    friend bool operator<(const Indexes &i1, const Indexes &i2) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i1.get(i).which() == 1 && boost::get<double>(i1.get(i)) >= boost::get<double>(i2.get(i)))
                return false;
            // todo: 这里将字符串比较转为int进行比较(ps: "10" < "5"
            // todo: 自定义比较
            if (i1.get(i).which() == 2) {
...
public:
    ...
    DBBPlusTree(int max_node_num = 5) : numberOfPointers(max_node_num) {}

    DataNode *get_root() {
        return rootBlock;
    }

    Indexes get_max_index() {
        // todo: be to test
        return _get_max_index(rootBlock);
    }

    Indexes _get_max_index(DataNode *node) {
        if (!node->parentBlock && node->node_num == 0)
            return Indexes("0");
        else {
            if (node->childBlock[node->node_num] != NULL)
                return _get_max_index(node->childBlock[node->node_num]);
            else
                return node->indexes[node->node_num - 1];
        }
    }
        ...
        if (curBlock->parentBlock == NULL) {
            ...
            for (i = 0; i <= curBlock->node_num; i++) {
                if (val < curBlock->indexes[i]) {
                    std::swap(curBlock->indexes[i], val);
                    std::swap(curBlock->data[i], data);
                }
            }
            ...
            for (i = 0; i < curBlock->node_num; i++) {
                if (newChildBlock->indexes[0] < curBlock->childBlock[i]->indexes[0]) {
                    std::swap(curBlock->childBlock[i], newChildBlock);
                }
            }
    ...
    std::vector<DataNode *> preorder_traversal(DataNode *node) {
        if (!node)
            return {};
        bool is_leaf = true;
        ...
        if (is_leaf) {
            ...
            std::vector<DataNode *> leafs;
            for (int i = 0; i < node->node_num + 1; i++) {
                if (node->childBlock[i] != NULL) {
                    std::vector<DataNode *> tmp = preorder_traversal(node->childBlock[i]);
    ...
    void save_to_file(std::string path) {
        auto fs = File::overwrite(path);
        std::ostream ss(fs.rdbuf());
        // std::ostream ss(std::cout.rdbuf());
        std::vector<DataNode *> res = preorder_traversal(rootBlock);
        int k = 1;
        for (int i = 0; i < res.size(); i++) {
            for (int j = 0; j < res[i]->node_num; j++) {
                // 第一行为索引
                ss << res[i]->indexes[j] << std::endl;
                // 第二行为值
                // ss << res[i]->data[j] << std::endl;
                for (auto it : res[i]->data[j])
                    ss << it << " ";
                ss << std::endl;
                k++;
            }
